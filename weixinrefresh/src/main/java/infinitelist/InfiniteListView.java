...
import com.zlit.weixinrefresh.R;
...
public class InfiniteListView extends ListView implements OnScrollListener {
	...
	private void init(Context pContext) {
		...
		mRefreshView = (ViewGroup) mInflater.inflate(R.layout.view_loadmore,
				this, false);
		mRefreshText = (TextView) mRefreshView.findViewById(R.id.refresh_label);
		mRefreshIcon = (ImageView) mRefreshView
		...
		mFlipAnimation = (RotateAnimation) AnimationUtils.loadAnimation(
				pContext, R.anim.anim_refresh);
	...
	public Parcelable onSaveInstanceState() {
		...
		bundle.putBoolean(IS_LOADING, mIsLoading.get()); // save the state of
															// the refresh view
		bundle.putParcelable(SUPER_STATE, super.onSaveInstanceState()); // save
																		// the
																		// super
																		// state
		return bundle;
	...
	public void onRestoreInstanceState(Parcelable pState) {
		if (pState instanceof Bundle) {
			...
			pState = bundle.getParcelable(SUPER_STATE); // if we receive a
														// superstate, we should
														// restore it

			mIsLoading.set(bundle.getBoolean(IS_LOADING));
			if (mIsLoading.get()) { // if the refresh view was displayed before
									// config change, it should be displayed
									// again
				showRefreshView();
			}
		}
	...
	public void onScroll(AbsListView view, int firstVisibleItem,
		...
		// The user must provide an OnListEnds object AND the list should not be
		// empty
		if (mOnListEnds != null && getAdapter() != null
				&& !getAdapter().isEmpty()) {

			if (!mIsLoading.get()
					&& mOnListEnds.isEnd(firstVisibleItem, visibleItemCount,
							totalItemCount)) {
				// show the refresh view IF it's not the case AND the end is
				// near
				showRefreshView();
	 ...
	 *            OnScrollListener - the new value
	 */
